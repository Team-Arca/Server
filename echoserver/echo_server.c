...
char main(int argc, char* argv[]) {
	...
	in_port_t recv_port = 9865;
	in_port_t send_port = 9866;
	...
	recv_addr.sin_addr.s_addr = htonl(recv_addr);	// 호스트의 어떠한 IP로도 연결 요청 수락
	...
	// 지역 주소에 바인드
	if(bind(recv_sock, (struct recv_addr*) &recv_addr, sizeof(recv_addr)) < 0)
		DieWithUserMessage("recv bind() failed");
	if(bind(send_sock, (struct send_addr*) &send_addr, sizeof(send_addr)) < 0)
...
void HandleTCPClient(int clientSock) {
	...
	while( numByteRcvd > 0) {
		...
		numByteRcvd = recv(clientSock, buffer, BUFSIZE, 0);
		if(numByteRcvd < 0)
		DieWithUserMessage("recv() failed");
	}
